...
public class BuddyController : MonoBehaviour
{
    ...
    private float buddyFallSpeed = -19f;  //バディの落下速度
    ...
    void Update()
    {
        ...
        else  //おんぶされてないときは通常通り重力働かせる＆物理演算の影響受けるように
        {
            ...
            float targetSpeed = moving ? 1f : 0f;  //走ってるかの判定
            float currentSpeed = animator.GetFloat(Const.Speed);  //スピードの現在値
    ...
    {
        float targetSpeed = isRunning ? 1f : 0f;  //走ってるかの判定
        float currentSpeed = animator.GetFloat(Const.Speed);  //スピードの現在値
        animator.SetFloat(Const.Speed, Mathf.MoveTowards(currentSpeed, targetSpeed, Time.deltaTime * 6f));
    ...
    private void OnTriggerExit(Collider other)
    {
        if (other.CompareTag("Player"))
        {
            buddyCarry.nearBuddy = false;  //Buddyが近くにいない
        }
    }

    //Animatorに指定した名前のパラメータが存在するかどうかを安全に確認する
